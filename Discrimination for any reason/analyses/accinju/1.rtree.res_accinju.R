##### runs the search for regression trees ###########
rm(list=ls(all=TRUE))
library(devtools)
library(tidyverse)
library(rpart)
...
dat <- readRDS("./01_Data for Analysis/Expanded sample (updated, August)/data_for_regTree_08132023.Rds") # 6281 observations (Study 3 only)
dim(dat)
...
#===============================================================================
### 1.- 'Classical' regression tree (exposure & covariates are all lump together as predictors)
#===============================================================================
summary(dat)
...
z <- c('female', 'agegrp', 'black', 'white', 'hisp', 'asian_nhpi', 'othrace', 'mhighgd_bin', 'rural', 'mixur'
       #'asian'    Asian collapsed into asian_nhpi (updated 8/11/2023)
       )
...
opcp  <- tree$cptable[, "CP"][which.min(tree$cptable[, "xerror"])] # Find the number of splits with minimized Xerror
...
{
  labs <- gsub("agegrp < 3.5", "Age < 30 ", labs)
  labs <- gsub("agegrp >= 3.5", "Age >= 30", labs)
  labs <- gsub("agegrp < 2.5", "Age < 25 ", labs)
  labs <- gsub("agegrp >= 2.5", "Age >= 25", labs)
}
prp(ptree, type = 4, # left and right split labels (see Figure 2)
    ...
    main = "Accidents and Injuries - Classical for Study 3", # main title
    cex.main = 1.2, # use big text for main title
    branch.col = "gray", # color of branch lines
    branch.lwd = 2, # line width of branch lines
    #prefix = "Proportion\n",
    roundint=FALSE, 
    faclen = 8,  # Increase the length of factor level names
    split.fun = split.fun
    ) 
png(
  paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/tree.plot.unconditional.", i, "_Study 3_labelsupdated.png"),
...
table(dat$node.cls, useNA = "always")
...
dat$W <-
  dat %>% 
  group_by(strata) %>%
  reframe(across(c(all_of(x)), ~ .x - weighted.mean(.x, w, na.rm = TRUE))) %>%   # reframe works only with dplyr version 1.1.0
  ungroup %>%
  select(-1)
...
png(
  paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/tree.plot.conditional.", i, "_Study 3.png"),
  width = 480 * 6, heigh = 480 * 4, res = 500)
rpart.plot(ptree.cond, roundint = FALSE)
dev.off()
...
split.fun <- function(x, labs, digits, varlen, faclen)  # YK added for manual edits the splits labels (4/5/2023)
{
  ...
  labs <- gsub("mloveaf < 0.5",  "Mother’s ACE- \nLack of parental \nlove and affection = No", labs)
  labs <- gsub("mloveaf >= 0.5", "Mother’s ACE- \nLack of parental \nlove and affection = Yes", labs)
  labs <- gsub("mphysab < 0.5",  "Mother’s ACE- \nPhysical abuse = No", labs)
  labs <- gsub("mphysab >= 0.5", "Mother’s ACE- \nPhysical abuse = Yes", labs)
  labs <- gsub("commstr < 0.5",  "Community \nstressors = No", labs)
  labs <- gsub("commstr >= 0.5", "Community \nstressors = Yes", labs)
  labs <- gsub("msubstu < 0.5",  "Mother's ACE- \nHH members with \nsubstance use problems = No", labs)
  labs <- gsub("msubstu >= 0.5", "Mother's ACE- \nHH members with \nsubstance use problems = Yes", labs)
  labs <- gsub("bneedin < 0.5",  "Basic needs \ninstability = No", labs)
  labs <- gsub("bneedin >= 0.5", "Basic needs \ninstability = Yes", labs)
  labs <- gsub("mmental >= 0.5", "Mother's ACE- HH members \nwith mental illness = Yes", labs)
  labs <- gsub("mmental < 0.5", "Mother's ACE- HH members \nwith mental illness = No", labs)
  labs <- gsub("ecstand >= 0.5", "Economic standing = Yes", labs)
  labs <- gsub("ecstand < 0.5", "Economic standing = No", labs)
...
prp(ptree_causal, type = 4, # left and right split labels (see Figure 2)
    ...
    leaf.round = 9, # round leaf nodes (for leaves, this supersedes the round arg)
    #prefix = "Depression Score\n", # prepend this string to the node labels
    main = "Accidents and Injuries - Causal for Study 3", # main title
    cex.main = 1.2, # use big text for main title
    branch.col = "gray", # color of branch lines
    branch.lwd = 2, # line width of branch lines
    roundint=FALSE, 
...
png(
  paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/tree.plot.causal", i, "_Study 3.png"),
