...
file_name <- paste0("./01_Data for Analysis/Expanded sample/NLS.tree_", out, ".Rds")
...
table(dat$ace_ocs, useNA = "ifany")   # ACE group is too small for contrast analysis. The contrast analyses were not conducted. 
...
dat$Z <- dat[, z]
# In order to balance the missing pattern we need to:
...
# here just overall mean, kids with average characteristics
tgt  <- colMeans(dat$C)
