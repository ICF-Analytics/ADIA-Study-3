...
tb %>%
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/predicted_cls", out, i, "_Study 3.html"))
...
fit.cnd <- svyglm(y ~ node.cnd
                  ...
                  #+ white - cannot control in drinkdy as it was identified in regression tree
                  + agegrp
                  + black  + hisp  + asian_nhpi + othrace +
                    + mhighgd_bin
                  + rural + mixur,
...
tb %>%
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/predicted_cau", out, i, "_Study 3.html"))
