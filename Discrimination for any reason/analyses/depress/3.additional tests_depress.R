...
left_join(tb_r, tb_w, by = "var", suffix = c(".raw", ".weighted")) %>%
  gt %>%
  tab_header(title = "Before and after weighting - Classical") %>%
  fmt_number(
    columns = everything(),
    rows = nrow(tb_r),
    decimals = 0,
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cls_raw_weighted", out, i, ".html"))
...
data.frame(Contrast = tests,
           coef(summary(fit0))[2:4, ]) %>%
  gt %>%
  tab_header(title = "Test - Classical") %>%
  fmt_number(
    columns = 2:5,
    rows = everything(),
    decimals = 3,
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cls_test0", out, i, ".html"))
...
left_join(freq, data.frame(Group = grp, pred0, confint(pred0)), by = "Group") %>%
  ...
  gt %>%
  tab_header(title = "Predicted values - Classical") %>%
  fmt_number(
    columns = 3:6,
    rows = everything(),
    decimals = 3,
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cls_pred0", out, i, ".html"))
...
data.frame(Contrast = tests,
           coef(summary(fit_dr))[2:4, ]) %>%
  gt %>%
  tab_header(title = "Test (Doubly robust) - Classical") %>%
  fmt_number(
    columns = 2:5,
    rows = everything(),
    decimals = 3,
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cls_test_dr", out, i, ".html"))
...
left_join(freq, data.frame(Group = grp, pred_dr, confint(pred_dr)), by = "Group") %>%
  rename(Estimate = link) %>%
  gt %>%
  tab_header(title = "Predicted values (Doubly robust) - Classical") %>%
  fmt_number(
    columns = 3:6,
    rows = everything(),
    decimals = 3,
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cls_pred_dr", out, i, ".html"))


# Save data for subgroup analysis
saveRDS(dat, file = paste0("./01_Data for Analysis/Expanded sample (updated, August)/data_with_weights_", out, "_cls.Rds"))


#-------------------------------------------------------------------
### Supplemental descriptive stats on the N of black/white/latino/etc  
### for each contrast group, and the median outcome score by race/ethnicity
#-------------------------------------------------------------------
library(tibble)

race <- as.data.frame(t(as.data.frame(crosstable(dat, c(ace_ocs), by=race)) %>% select(-'.id', -label, -'NA'))) 
names(race) <- as.matrix(race[1,])
race <- tibble::rownames_to_column(race, "VALUE") %>% 
  mutate(Label1 = "Race") %>% 
  select(-'NA') %>% 
...
#medu <- as.data.frame(t(as.data.frame(crosstable(dat, c(ace_ocs), by=mhighgd_bin)) %>% select(-'.id', -label))) 
#names(medu) <- as.matrix(medu[1,])
#medu <- tibble::rownames_to_column(medu, "VALUE") %>% mutate(demo = "Mother Education") %>% select(-'NA') %>% filter(VALUE != "variable")

overall <- as.data.frame(t(tapply(dat[[out]], dat$ace_ocs, mean, na.rm = T))) %>% 
  mutate(Label1 = "Outcome Score (overall mean)", VALUE = "", Label2 = "") %>%
  mutate_if(is.numeric, format, nsmall = 3)

outrace <- as.data.frame(t(tapply(dat[[out]], list(dat$ace_ocs, dat$race), mean, na.rm = T))) %>% 
  mutate(Label1 = "Outcome Score (mean by race)")
outrace <- tibble::rownames_to_column(outrace, "VALUE") %>%
  mutate(Label2 = case_when(
    VALUE == 1 ~ "Hispanic", 
    VALUE == 2 ~ "White", 
    VALUE == 3 ~ "Black", 
...
all <- rbind(race, overall, outrace) %>% select(-VALUE)
all <- all[, c("Label1", "Label2", "None", "ACE", "ACE + OCS", "OCS")] 


data.frame(all) %>%
  gt %>%
  tab_header(title = "Race/Ethnicity Composition and Outcome Score - Classical") %>%
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cls_supplemental_race_", out,".html"))
...
dat <-
  ...
  mutate(ace_ocs =
           case_when(
             anyACE_T == 1 & !(loveaff == 1 | bneedin == 1) ~ "ACE"
             , anyACE_T == 1 &  (loveaff == 1 | bneedin == 1) ~ "ACE + OCS"
...
left_join(tb_r, tb_w, by = "var", suffix = c(".raw", ".weighted")) %>%
  gt %>%
  tab_header(title = "Before and after weighting - Causal") %>%
  fmt_number(
    columns = everything(),
    rows = nrow(tb_r),
    decimals = 0,
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cau_raw_weighted", out, i, ".html"))
...
data.frame(Contrast = tests,
  ...
  #gtsave(paste0("C:/Users/55231/OneDrive - ICF/Desktop/ADIA/output/test0", out, i, ".html"))
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cau_test0", out, i, ".html"))
...
left_join(freq, data.frame(Group = grp, pred0, confint(pred0)), by = "Group") %>%
  rename(Estimate = link) %>%
  gt %>%
  tab_header(title = "Predicted values - Causal") %>%
  fmt_number(
    columns = 3:6,
    rows = everything(),
    decimals = 3,
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cau_pred0", out, i, ".html"))
...
data.frame(Contrast = tests,
           coef(summary(fit_dr))[2:4, ]) %>%
  gt %>%
  tab_header(title = "Test (Doubly robust) - Causal") %>%
  fmt_number(
    columns = 2:5,
    rows = everything(),
    decimals = 3,
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cau_test_dr", out, i, ".html"))
...
pred_dr <- predict(fit_dr,
                   newdata = data.frame(ace_ocs = grp,
                                        ...
                                        white = 0,
                                        hisp = 1,
                                        # asian = 0,  Asian collaped into asian_nhpi (8/11/2023)
                                        asian_nhpi = 0,
                                        othrace = 0,
                                        mhighgd_bin = 0,
                                        rural = 0,
...
left_join(freq, data.frame(Group = grp, pred_dr, confint(pred_dr)), by = "Group") %>%
  rename(Estimate = link) %>%
  gt %>%
  tab_header(title = "Predicted values (Doubly robust) - Causal") %>%
  fmt_number(
    columns = 3:6,
    rows = everything(),
    decimals = 3,
  ...
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cau_pred_dr", out, i, ".html"))
...
data.frame(all) %>%
  gt %>%
  tab_header(title = "Race/Ethnicity Composition and Outcome Score - Causal") %>%
  gtsave(paste0("./05_Regression Tree Output/03 Race updated (August)/", out, "/cau_supplemental_race_", out,".html"))
