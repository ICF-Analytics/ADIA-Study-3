...
## this tests the contrasts between individuals included and individuals excluded in Study 3 the causal model
...
n   <- svyby(~y, ~ ace_ocs + d
             , sdw2,  unwtd.count, na = TRUE)    # These counts exclude the individuals with zero weights
merge(est, n[, -ncol(n)])
merge(est, n[, -ncol(n)]) %>%
  gt %>%
...
#sum(dat2$new_w<=0)
#dat3 <- filter(dat2, is.na(dat2$y)==F & dat2$new_w > 0)
#table(dat3$ace_ocs, dat3$d, useNA = "always")
