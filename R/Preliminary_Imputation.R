...
set.seed(3333)
dat <- dat %>%
  mutate(testing.sample = 
           if_else(childid %in% sample(unique(childid), round(n_distinct(childid)/5)), 1, 0)
  )
table(dat$testing.sample, useNA = "always")


# Recode predictors
predictors <- c("ygender_xrnd", "black", "white", "hisp", "asian", "asian_nhpi", "othrace", "mhighgd_bin",
                #"mhhinco_adj",                  # This is continuous variable and processed separately
                "urbnrur", "lastgrd_bin")
for (i in predictors){
print(i)
...
test <- filter(dat, testing.sample == 1)
train <- filter(dat, testing.sample == 0)
...
## Using optimal Cut-off
optimalCutoff(test$discrim_reason, test$predicted)
# Confusion Matrix
confusionMatrix(actual=test$discrim_reason, predicted=test$predicted, threshold = optimal) 
sensitivity(test$discrim_reason, test$predicted, threshold = optimal)
# Specificity
specificity(test$discrim_reason, test$predicted, threshold = optimal)
# Misclassification Error
misClassError(test$discrim_reason, test$predicted, threshold = optimal)


## Using 0.5 as cut-off
# Confusion Matrix
