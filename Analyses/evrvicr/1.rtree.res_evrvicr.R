...
{
  labs <- gsub("msubstu < 0.5", "msubstu = no", labs)
  labs <- gsub("msubstu >= 0.5", "msubstu = yes", labs)
  labs <- gsub("mphysab < 0.5", "mphysab = no", labs)
  labs <- gsub("mphysab >= 0.5", "mphysab = yes", labs)
  labs <- gsub("ecstand < 0.5", "ecstand = no", labs)
  labs <- gsub("ecstand >= 0.5", "ecstand = yes", labs)
  labs <- gsub("commstr < 0.5", "commstr = no", labs)
  labs <- gsub("commstr >= 0.5", "commstr = yes", labs)
  labs <- gsub("bneedin < 0.5", "bneedin = no", labs)
  labs <- gsub("bneedin >= 0.5", "bneedin = yes", labs)
}
