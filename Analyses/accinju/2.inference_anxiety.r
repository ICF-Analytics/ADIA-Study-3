...
tb %>%
  ...
  tab_footnote(footnote = paste0("P-value=", test.cls$p, "; df=", test.cls$df, "; denominator df=", test.cls$ddf),  cells_stubhead()) %>% # added to save p-value 
  gtsave(paste0("./Regression Tree Output/predicted_cls", out, i, "_Study 3.html"))
...
pred.cnd <- predict(fit.cnd,
                    newdata = data.frame(node.cnd = unique(dat$node.cnd),
                                         female = 1, #female identified in tree so commenting out
...
tb <- tb[ , c(1, 5, 2:4)]
colnames(tb) <- c("Node", "n", "Est.", "95%CI_LL", "95%CI_UL")

#saving the output in nice format
tb %>%
  gt %>%
  tab_header(title = "Predicted values") %>%
  tab_footnote(footnote = paste0("P-value=", test.cau$p, "; df=", test.cau$df, "; denominator df=", test.cau$ddf),  cells_stubhead()) %>% # added to save p-value 
  gtsave(paste0("./Regression Tree Output/predicted_cau", out, i, "_Study 3.html"))
