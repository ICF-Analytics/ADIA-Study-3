...
#dat_s1 <- readRDS("C:/Users/55231/OneDrive - ICF/Desktop/ADIA/data/finalvar.Rds")
#names(dat_s1)  # 11,545 observations #
#dat_s1
...
dat <- subset(org_dat, discrim_ethn==1 | discrim_race==1 | discrim_raceethn==1 | discrim_sexualorient==1) # This is same with discrim_reason = 1
#dat <- org_dat
...
{
  labs <- gsub("agegrp < 3.5", "agegrp < 30 ", labs)
  labs <- gsub("agegrp >= 3.5", "agegrp >= 30", labs)
}
...
split.fun <- function(x, labs, digits, varlen, faclen)  # YK added for manual edits the splits labels (4/5/2023)
{
  labs <- gsub("loveaff < 0.5", "loveaff = no", labs)
  labs <- gsub("loveaff >= 0.5", "loveaff = yes", labs)
  labs <- gsub("mphysab < 0.5", "mphysab = no", labs)
  labs <- gsub("mphysab >= 0.5", "mphysab = yes", labs)
}
...
png(
  paste0("./Regression Tree Output/", out, "/tree.plot.causal", i, "_Study 3_labelsupdated.png"),
  width = 480 * 4, heigh = 480 * 4, res = 300)
rpart.plot(ptree_causal, roundint = FALSE)
prp(ptree_causal, type = 4, # left and right split labels (see Figure 2)
    clip.right.labs = FALSE, # full right split labels
